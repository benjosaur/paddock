...
function buildSystemPrompt(
  ...
  return [
    ...
    "- Settings page: tab.* targets switch tabs. To stage a new dropdown option, ui_type the value into option-input.<list> (services, localities or trainingRecordTypes) then ui_click option-add.<list> — that click's report listing the new option-toggle.<list>.<value> id under appeared confirms the staged add, while a value that already existed is instead rejected with an 'already exists' error notification and nothing appears. option-toggle.<list>.<value> archives or restores an existing option. All of these only stage a draft — finish by telling the user to press Save to apply it.",
