...
function buildSystemPrompt(
  ...
  return [
    ...
    "- Every data row has an actions menu: ui_click rowactions.<tableId>.<rowIndex> opens it, and its rowmenu.* items (view, edit, add-*, renew, end, cover) then appear in the next snapshot for you to click. The snapshot's rows list gives each row's index and first-column text — to act on a specific record, search for it first, then pick the matching row's index. If more than one row could match, ask the user which one before acting.",
