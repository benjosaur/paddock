...
import type { Mp, TrainingRecordItem } from "../types";
...
export function MpForm() {
  ...
  const [trainingInput, setTrainingInput] = useState({ training: "", expiry: "" });

  const handleInputChange = (field: keyof Mp, value: string | number) => {
    setFormData(prev => ({ ...prev, [field]: value }));
  };

  const handleArrayInputChange = (field: "servicesOffered" | "specialisms", value: string) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="nextOfKin" className="block text-sm font-medium text-gray-700 mb-1">
                  Next of Kin
                </label>
                <Input
                  id="nextOfKin"
              ...
              <div>
                <label htmlFor="dbsNumber" className="block text-sm font-medium text-gray-700 mb-1">
                  DBS Number
                </label>
                <Input
                  id="dbsNumber"
                  value={formData.dbsNumber || ""}
                  onChange={(e) => handleInputChange("dbsNumber", e.target.value)}
                />
              </div>

              <div>
                <label htmlFor="dbsExpiry" className="block text-sm font-medium text-gray-700 mb-1">
                  DBS Expiry
                </label>
                <Input
                  id="dbsExpiry"
                  ...
                  onChange={(e) => handleInputChange("dbsExpiry", e.target.value)}
                />
              </div>
            </div>

            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-700">Services & Capacity</h3>
              ...
              <div>
                <label htmlFor="specialisms" className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <div className="space-y-3">
                <h4 className="text-md font-medium text-gray-700">Training Records</h4>
                
                <div className="flex space-x-2">
                  <Input
                    placeholder="Training name"
                    value={trainingInput.training}
                    onChange={(e) => setTrainingInput(prev => ({ ...prev, training: e.target.value }))}
                  />
                  <Input
                    type="date"
                    placeholder="Expiry date"
                    value={trainingInput.expiry}
                    onChange={(e) => setTrainingInput(prev => ({ ...prev, expiry: e.target.value }))}
                  />
                ...
                {formData.trainingRecords && formData.trainingRecords.length > 0 && (
